...
<li class="">
    ...
    <div class="content-wrapper">
        ...
        <div>
    <a style="margin-bottom: 20px; float: right; padding: 2px 30px 2px 30px; font-size: 14px;" href="{{ route('accounts.payments.create')}}" class="btn btn-primary">Add</a>
    <a style="margin-bottom: 20px; float: right; padding: 2px 30px 2px 30px; margin-right: 4px; font-size: 14px;" href="{{ url('accounts/payments/index/pdf') }}" class="btn btn-danger">PDF</a>
</div>  
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr class="tr-t-1">
                        <td class="tr-t-nm">{{ $payment->date }}</td>
                        <td style="word-wrap: break-word;min-width: 160px;max-width: 160px;">{{ $payment->details }}</td>
                        <td class="tr-t-nm">{{ $payment->amount }}</td>
                        <th>
                        <a href="{{ route('accounts.payments.edit', $payment->id) }}" class="float-left">
                                <button>
                                    <i class="fa fa-pencil ico-tb"></i>
                                </button>
                            </a>
                            <form action="{{ route('accounts.payments.destroy', $payment->id) }}" method="POST"
                                class="float-left">
