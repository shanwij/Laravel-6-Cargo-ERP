        ...
        <div class="row">
            <div class="col">
                <div>
                    ...
                    @endif
                    <form method="post" action="{{ route('accounts.payments.update', ['payment' => $payment->id]) }}">
                    @method('PATCH') 
                    @csrf
                        <div class="form-group">
                            <label for="date">Date:</label>
                            <input type="text" class="form-control" name="date" value={{ $payment->date }} />
                        </div>

                        <div class="form-group">
                            <label for="desc">Details:</label>
                            <textarea class="form-control" name="details" rows="5" >{{ $payment->details }}</textarea>
                        </div>
