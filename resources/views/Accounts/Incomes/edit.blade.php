        ...
        <div class="row">
            <div class="col">
                <div>
                    ...
                    @endif
                    <form method="post" action="{{ route('accounts.incomes.update', ['income' => $income->id]) }}">
                    @method('PATCH') 
                    @csrf
                        <div class="form-group">
                            <label for="date">Date:</label>
                            <input type="text" class="form-control" name="date" value={{ $income->date }} />
                        </div>

                        <div class="form-group">
                            <label for="desc">Details:</label>
                            <textarea class="form-control" name="details" rows="5" >{{ $income->details }}</textarea>
                        </div>
