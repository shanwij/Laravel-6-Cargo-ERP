...
<li class="">
    ...
    <div class="content-wrapper">
        ...
        <div class="con-tab">
            ...
            <div>
                <a style="margin-bottom: 20px; float: right; padding: 2px 30px 2px 30px; font-size: 14px;"
                    href="{{ route('customers.tasks.create') }}" class="btn btn-primary">Add</a>
                    <a style="margin-bottom: 20px; float: right; padding: 2px 30px 2px 30px; margin-right: 4px; font-size: 14px;" href="{{ url('customers/tasks/index/pdf') }}" class="btn btn-danger">PDF</a>
            </div>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach($taskNotes as $taskNotes)
                    <tr class="tr-t-1">
                        <td class="tr-t-nm">{{$taskNotes->id}}</td>
                        <td class="tr-t-nm">{{$taskNotes->contact['Company']}}</td>
                        <td class="tr-t-nm">{{$taskNotes->description}}</td>
                        <td class="tr-t-nm">{{$taskNotes->Todo_Due_Date}}</td>
                        <td class="tr-t-nm">{{$taskNotes->Task_Update}}</td>
                        <td class="tr-t-nm">{{$taskNotes->Sales_Rep}}</td>
                        <td class="tr-t-nm">{{$taskNotes->updated_at}}</td>
                        <td class="tr-t-nm">{{$taskNotes->Task_Status}}</td>
                        <th>
                            <a href="{{ route('customers.tasks.edit', $taskNotes->id) }}" class="float-left">
                                <button>
                                    <i class="fa fa-pencil ico-tb"></i>
                                </button>
                            </a>
                            <form action="{{ route('customers.tasks.destroy', $taskNotes->id) }}" method="POST"
                                class="float-left">
